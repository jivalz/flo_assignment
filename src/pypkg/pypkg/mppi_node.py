...
def laserscan_to_world_frame(scan:LaserScan, rb_x : float,rb_y : float,rb_yaw : float, max_range : float=4.0,min_range : float=0.05,) -> np.ndarray:
...
class mppi:
    ...
    def calculate_cost(self, x_sampled:np.ndarray, u_sampled:np.ndarray, scans:np.ndarray, current_idx: int):
        ...
        # Query closest points against ONLY the local window
        local_kdtree = cKDTree(local_path)
        _, local_idx = local_kdtree.query(pts)
        closest_idx = local_idx + window_start
...
class mppinode(Node):
    def __init__(self):
        ...
        if not wp_file:
            wp_file = os.path.expanduser('~/assignment/src/pypkg/csv/waypoint2.csv')
            if not os.path.exists(wp_file):
                wp_file = os.path.expanduser('~/assignment/src/nav/waypoints/waypoint2.csv')
        ...
        self.solver = mppi(
            ...
            K=500,
            N=50,  # Increased horizon so it can see through the U-turn
            dt=0.05,
            lambda_=50.0, # Increased temperature to prevent mode collapse
            wheading=1.0, # Drastically reduced so it doesn't get paralyzed at sharp bends
            wtracking=15.0,
